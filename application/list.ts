...
export const getProductList = cache(async (filter:QueryGet_Product_ListArgs):Promise<List|null>=>{
    const verifyCallback = new RequestBuilder ("GET",Uri.shopServiceIP+'/products/list?code='+filter?.code??null+"&skip="+filter?.skip??0+"&limit="+filter?.limit??10).addHeader( "Content-Type","application/json").ExecuteAsync()
